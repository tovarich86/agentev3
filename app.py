...
GITHUB_RELEASE_URL = "https://github.com/tovarich86/agentev2/releases/download/V1.0-data/data_release.zip"
...
# --- CARREGADOR DE DADOS ATUALIZADO ---
...
def setup_and_load_data():
    ...
    if not all((CACHE_DIR / f).exists() for f in expected_files):
        logger.info(f"Baixando artefatos de {GITHUB_RELEASE_URL}...")
...
def _create_flat_alias_map(kb: dict) -> dict:
    """Helper interno para criar um mapa de alias -> nome canônico."""
    alias_to_canonical = {}
...
# Gera a lista de tópicos disponíveis para o fallback do LLM
...
def expand_search_terms(base_term: str, kb: dict) -> list[str]:
    """ADAPTADO: Expande um termo de busca usando o DICIONARIO_UNIFICADO_HIERARQUICO."""
...
def search_by_tags(artifacts: dict, company_name: str, target_tags: list) -> list:
    """ADAPTADO: Busca por chunks que contenham metadados no novo formato [topico:...]."""
    results = []
    ...
    for index_name, artifact_data in artifacts.items():
        ...
        for i, mapping in enumerate(chunk_map):
            # O nome da empresa agora está no mapa do chunk
            if searchable_company_name in mapping.get("company_name", "").lower():
                ...
                if found_topics_in_chunk:
                    ...
                    if intersection:
                        results.append({
                            'text': chunk_text, 'path': mapping.get('source_url', 'N/A'), 'index': i,
                            'source': index_name, 'tag_found': ','.join(intersection)
                        })
    return results

# --- LÓGICA ROBUSTA ORIGINAL (PRESERVADA E ADAPTADA) ---

def get_final_unified_answer(query: str, context: str) -> str:
    """PRESERVADO: Gera a resposta final usando a lógica de prompt detalhada original."""
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent?key={GEMINI_API_KEY}"
    
    # Esta lógica de detecção agora opera sobre o contexto limpo
...
def execute_dynamic_plan(plan: dict, artifacts: dict, model, kb: dict) -> tuple[str, set]:
    """PRESERVADO E ADAPTADO: Executa o plano de busca com a lógica robusta original."""
...
def create_dynamic_analysis_plan(query, company_catalog_rich, kb):
    """PRESERVADO E ADAPTADO: Usa a lógica de planejamento original."""
    query_lower = query.lower().strip()
    
    # Lógica de identificação de empresa original
    mentioned_companies = []
    ...
    # Lógica de identificação de tópicos adaptada
    ...
    if not topics:
        ...
        try:
            llm_response = get_final_unified_answer(prompt, "") # Usa a mesma função de chamada
...
def handle_rag_query(query, artifacts, model, kb, company_catalog_rich):
    """PRESERVADO E ADAPTADO: Handler de RAG original e robusto."""
